#!/usr/bin/env python3
"""
All-in-one launcher + real-time monitor for a full parallel recon scan.

Usage:  python _sugarrushed_scan_launch_monitor.py
Output: _sugarrushed_console.log       (full raw NDJSON console feed)
        _sugarrushed_issues.txt        (issues / errors / failures only)
        _sugarrushed_summary.txt       (final summary)
"""
...
LOG_FILE    = "_sugarrushed_console.log"
ISSUE_FILE  = "_sugarrushed_issues.txt"
SUMMARY_FILE= "_sugarrushed_summary.txt"
...
FALSE_POSITIVE_PATTERNS = [
    ...
    # sugarrushed-specific / Shopify GraphQL schema enumeration (verbose but expected)
    "baseline responses differ",   # non-deterministic target — expected
    "no clear majority",
    "confidence: 80% │ source: graphql",  # Shopify GraphQL schema fields — verbose but normal
    "│ source: graphql │",
]
...
def wait_for_health(port: int, timeout: float = 30.0) -> bool:
    ...
    while time.time() < deadline:
        try:
            with urllib.request.urlopen(f"http://localhost:{port}/health", timeout=1.5) as r:
                if r.status == 200:
                    return True
        except Exception:
            pass
...
def main() -> None:
    print("=" * 68)
    print(f"  CaseCrack Full Parallel Recon — {TARGET}")
    ...
    if not wait_for_health(HTTP_PORT, timeout=45.0):
        print(f"[{ts()}] ERROR: Dashboard did not start within 45s — check dashboard_stderr.log", flush=True)
        proc.terminate()
    ...
    if tok_resp and "token" in tok_resp:
        ...
        print(f"[{ts()}] Note: no auth token required or fetch failed: {tok_resp}", flush=True)

    # ── 3. Start scan ─────────────────────────────────────────────────────────
    print(f"[{ts()}] Starting full parallel scan against {TARGET} ...", flush=True)
    # First try /api/scan/start
    start_resp = api("/api/scan/start", method="POST",
                     body={"target": TARGET}, token=token)
    if start_resp and start_resp.get("ok"):
        print(f"[{ts()}] Scan started via /api/scan/start: {start_resp}", flush=True)
    elif start_resp and start_resp.get("status") == "running":
        print(f"[{ts()}] Scan already running (auto-started with target)", flush=True)
    elif start_resp and "already running" in str(start_resp).lower():
        print(f"[{ts()}] Scan already running (auto-started)", flush=True)
    else:
        print(f"[{ts()}] /api/scan/start response: {start_resp}", flush=True)
        # Try standalone start endpoint
        alt = api("/api/standalone/start", method="POST",
                  body={"target": TARGET}, token=token)
        print(f"[{ts()}] /api/standalone/start response: {alt}", flush=True)
    ...
    with open(ROOT / LOG_FILE, "w", encoding="utf-8") as log_fh, \
        ...
        issue_fh.write(
            f"=== SugarRushed Scan Issue Report ===\n"
            f"=== Target: {TARGET} ===\n"
        ...
        while time.monotonic() < deadline_mono:
            ...
            total        = len(phase_status) or status.get("total_phases", status.get("phases_total", 36))
    ...
    with open(ROOT / SUMMARY_FILE, "w", encoding="utf-8") as sf:
        sf.write(f"{'='*80}\n")
        sf.write(f"FULL PARALLEL RECON — {TARGET}\n")
        ...
        for evt in phase_completion_events:
            sf.write(f"  {evt['phase']:40s}  {evt['outcome']}\n")
        if not phase_completion_events:
            sf.write("  (no phase completion events received)\n")
    ...
    # Keep dashboard running (user might want to browse results)
