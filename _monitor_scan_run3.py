#!/usr/bin/env python3
"""
Real-time scan monitor — run 3 (2026-05-04)
Pulls console feed and records all errors, warnings, failures,
and degraded phases.

Run:  python _monitor_scan_run3.py
Output: _scan_monitor_run3.log  (all events)
        _scan_issues_run3.txt   (issues/errors only)
"""
...
LOG_FILE   = "_scan_monitor_run3.log"
ISSUE_FILE = "_scan_issues_run3.txt"
...
FALSE_POSITIVE_PATTERNS = [
    ...
    "soft-timeout",
    "soft limit",
    # FIX-FP-4: Queue/skip informational messages
    "tests queued",       # "42 tests queued, 6 skipped" — informational
    "skipped tests",      # summary line
    "skipped (",          # "skipped (N)" in summary counts
    # FIX-FP-5: WAF/rate limit expected behaviour
    "waf/rate-limit",
    "rate-limit hit",
    # FIX-FP-6: Passive observations not errors
    "passive observation",
    "passively observed",
]
...
def main() -> None:
    ...
    print(f"[{ts()}] Monitor run3 started — polling http://localhost:8770", flush=True)
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as log_fh, \
        ...
        issue_fh.write(f"=== Scan Issue Report Run3 — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")
        ...
        while True:
            ...
            for line in lines:
                ...
                if stream == "stderr":
                    ...
                    print(f"[{ts()}] [ISSUE][{stream}][{phase}] {text[:200]}", flush=True)
                    rec = {"ts": line_ts, "phase": phase, "stream": stream, "text": text}
                    issues.append(rec)
                    phase_issues[phase].append(text)
