"""Inject EVENTBUS + TIER2 + TIER4A hardening blocks into all Sprint 6 output modules."""
...
SPRINT6_MODULES = [
    ("output", "cvss_calculator"),
    ("output", "evidence_collector"),
    ("output", "json_report"),
    ("output", "remediation_advisor"),
    ("output", "sarif_output"),
    ("output", "assessment_templates"),
    ("output", "attack_narrative"),
    ("output", "chain_render"),
...
for subdir, name in SPRINT6_MODULES:
    ...
    # Find injection point: after the last `import` block but before module-level code
    # We'll insert right after the `logger = ...` line (standard in all Sprint 6 modules)
    ...
    if inject_after == -1:
        ...
        for i, line in enumerate(lines):
            if line.startswith("import ") or line.startswith("from "):
                inject_after = i
        print(f"  [WARN fallback injection point] {subdir}/{name}.py at line {inject_after + 1}")
    else:
        print(f"  [OK inject after logger line {inject_after + 1}] {subdir}/{name}.py")

    # Build the hardening block
    hardening = "\n" + EVENTBUS_BLOCK + TIER2_BLOCK + tier4a_block(name)

    # Insert after inject_after line
