#!/usr/bin/env python3
"""
Real-time scan monitor — pulls console feed and records all
errors, warnings, failures, and degraded phases.

Run:  python _monitor_scan_20260503.py
Output: _scan_monitor_20260503.log  (all events)
        _scan_issues_20260503.txt   (issues/errors only)
"""

import json
import sys
import time
import urllib.request
from collections import defaultdict
from datetime import datetime, timezone

# Ensure stdout can handle arbitrary Unicode on Windows cp1252 terminals
if hasattr(sys.stdout, "reconfigure"):
    try:
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    except Exception:
        pass

TOKEN = "mpogZXAS82-WoTm6_5evKtWQlQUIt23KVCxTV73qoY0"
BASE = "http://localhost:8770"
HEADERS = {"Authorization": f"Bearer {TOKEN}"}

LOG_FILE   = "_scan_monitor_20260503_run2.log"
ISSUE_FILE = "_scan_issues_20260503_run2.txt"

ISSUE_KEYWORDS = [
    "traceback", "exception", "attributeerror", "typeerror",
    "valueerror", "keyerror", "importerror", "filenotfounderror",
    "oserror", "runtimeerror", "indexerror", "nameerror",
    "failed", "failure", "degraded", "crash",
    "unrecognized arguments", "no such file", "cannot", "unable to",
    "timed out", "connection refused", "connection error",
    "ssl error", "no module named", "broken pipe",
    "no cli handler",
]
# Exact-match keywords (whole-word, not substring) to reduce false positives
ISSUE_KEYWORDS_EXACT = [
    "error", "failed", "timeout", "warning", "missing",
    "skipped", "abort", "warn",
]
# Patterns in text that indicate progress/info, NOT errors — suppress even
# when an issue keyword appears as a substring.
FALSE_POSITIVE_PATTERNS = [
    "[dorking]",          # dorking progress lines: "0 errors | 0 rate-limits"
    "q/s eta=",           # ETA progress lines
    "queries |",          # query counter progress
    "cache hits (skipped)",  # normal cache operation
    "| 0 errors |",       # zero-error stat in progress line
    "categories:",        # category summary (includes 'error-pages')
    "error-pages:",       # category label in summary
    # FIX-FP-1: JSON data lines from Exploit Graph Analysis phase output.
    # The exploit graph tool emits structured JSON with field names like
    # "failure_count" and "last_failure_at" to stderr; these contain the
    # keyword "failure" which triggers the issue detector, but they are
    # normal structured data showing zero failures (not actual errors).
    '"failure_count"',    # JSON field: "failure_count": 0 (exploit graph)
    '"last_failure_at"',  # JSON field: "last_failure_at": null (exploit graph)
    # FIX-FP-2: OOB/callback polling messages are expected tool behaviour.
    # These are progress messages printed while the tool waits for out-of-band
    # interactions (e.g. DNS callbacks, collaborator pings). They are
    # informational and matched on "timeout" in the text body.
    "polling for oob interactions",   # SSRF/injection OOB polling
    "polling for callbacks",          # Collaborator callback polling
    # FIX-FP-3: Soft-timeout is normal phase flow control, not a failure.
    # The runner emits SOFT-TIMEOUT when a command exceeds its soft budget
    # but continues running — this is expected and handled gracefully.
    "soft-timeout",                   # phase soft-timeout progress marker
    "soft limit",                     # "soft limit Xs reached — evaluating…"
]


def api_get(path: str) -> dict:
    req = urllib.request.Request(BASE + path, headers=HEADERS)
    with urllib.request.urlopen(req, timeout=30) as r:
        return json.loads(r.read())


def is_issue(text: str) -> bool:
    low = text.lower()
    # Suppress known false-positive progress/info patterns first
    for fp in FALSE_POSITIVE_PATTERNS:
        if fp in low:
            return False
    # Primary: substring match on unambiguous keywords
    if any(kw in low for kw in ISSUE_KEYWORDS):
        return True
    # Secondary: exact-word match for ambiguous keywords
    import re
    for kw in ISSUE_KEYWORDS_EXACT:
        if re.search(r'\b' + re.escape(kw) + r'\b', low):
            return True
    return False


def ts() -> str:
    return datetime.now(timezone.utc).strftime("%H:%M:%S")


def main() -> None:
    since = 0
    issues: list[dict] = []
    phase_issues: defaultdict[str, list[str]] = defaultdict(list)
    seen_ids: set[str] = set()

    print(f"[{ts()}] Monitor started — polling http://localhost:8770", flush=True)
    print(f"[{ts()}] Log  -> {LOG_FILE}", flush=True)
    print(f"[{ts()}] Issues -> {ISSUE_FILE}", flush=True)
    print(flush=True)

    poll_interval = 3.0
    consecutive_complete_polls = 0

    with open(LOG_FILE, "w", encoding="utf-8") as log_fh, \
         open(ISSUE_FILE, "w", encoding="utf-8") as issue_fh:

        issue_fh.write(f"=== Scan Issue Report — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n\n")

        while True:
            # ── Fetch new console lines ────────────────────────────────
            try:
                data = api_get(f"/api/standalone/console?limit=500&since={since}")
                lines = data.get("lines", [])
                next_since = data.get("next_since", since)
            except Exception as exc:
                print(f"[{ts()}] Console fetch error: {exc}", flush=True)
                time.sleep(poll_interval)
                continue

            for line in lines:
                uid = f"{line.get('ts',0)}-{line.get('phase','')}-{line.get('text','')[:40]}"
                if uid in seen_ids:
                    continue
                seen_ids.add(uid)

                phase  = line.get("phase", "")
                stream = line.get("stream", "")
                text   = line.get("text", "")
                line_ts = line.get("ts", 0)

                # Write to full log
                log_fh.write(json.dumps(line) + "\n")
                log_fh.flush()

                # Print stderr always + check for issues
                if stream == "stderr":
                    print(f"[{ts()}] [stderr][{phase}] {text[:160]}", flush=True)
                    if is_issue(text):
                        rec = {"ts": line_ts, "phase": phase, "stream": stream, "text": text}
                        issues.append(rec)
                        phase_issues[phase].append(text)
                        issue_fh.write(f"[{phase}] [stderr] {text}\n")
                        issue_fh.flush()

                elif stream == "cmd":
                    print(f"[{ts()}] [CMD][{phase}] {text[:160]}", flush=True)

                elif stream in ("log", "event") and is_issue(text):
                    print(f"[{ts()}] [ISSUE][{stream}][{phase}] {text[:160]}", flush=True)
                    rec = {"ts": line_ts, "phase": phase, "stream": stream, "text": text}
                    issues.append(rec)
                    phase_issues[phase].append(text)
                    issue_fh.write(f"[{phase}] [{stream}] {text}\n")
                    issue_fh.flush()

            if next_since > since:
                since = next_since

            # ── Check scan status ──────────────────────────────────────
            try:
                status = api_get("/api/standalone/status")
            except Exception as exc:
                print(f"[{ts()}] Status fetch error: {exc}", flush=True)
                time.sleep(poll_interval)
                continue

            is_complete  = status.get("is_complete", False)
            completed    = status.get("completed_phases", 0)
            total        = len(status.get("phase_status", {})) or status.get("total_phases", 36)
            findings_cnt = status.get("findings_count", 0)
            current      = status.get("current_phase", "")
            phase_statuses = status.get("phase_status", {})

            # Collect degraded phases
            for ph, st in phase_statuses.items():
                if st == "degraded":
                    msg = f"Phase DEGRADED: {ph}"
                    uid2 = f"degraded-{ph}"
                    if uid2 not in seen_ids:
                        seen_ids.add(uid2)
                        issues.append({"phase": ph, "stream": "status", "text": msg})
                        phase_issues[ph].append(msg)
                        issue_fh.write(f"[{ph}] [status] {msg}\n")
                        issue_fh.flush()
                        print(f"[{ts()}] *** {msg} ***", flush=True)

            print(
                f"[{ts()}] Status: {completed}/{total} complete | findings={findings_cnt}"
                + (f" | current={current}" if current else ""),
                flush=True,
            )

            if is_complete:
                consecutive_complete_polls += 1
                if consecutive_complete_polls >= 2:
                    print(f"\n[{ts()}] === SCAN COMPLETE ===", flush=True)
                    break
            else:
                consecutive_complete_polls = 0

            time.sleep(poll_interval)

    # ── Final summary ─────────────────────────────────────────────────
    print(f"\n{'='*60}", flush=True)
    print(f"ISSUE SUMMARY — {len(issues)} total issues found", flush=True)
    print(f"{'='*60}", flush=True)

    if not issues:
        print("  No issues detected!", flush=True)
    else:
        for phase, msgs in sorted(phase_issues.items()):
            print(f"\n  [{phase}] — {len(msgs)} issue(s):", flush=True)
            for m in msgs[:5]:
                print(f"    • {m[:120]}", flush=True)
            if len(msgs) > 5:
                print(f"    ... and {len(msgs)-5} more", flush=True)

    with open(ISSUE_FILE, "a", encoding="utf-8") as fh:
        fh.write(f"\n\n=== SUMMARY: {len(issues)} issues found ===\n")
        for phase, msgs in sorted(phase_issues.items()):
            fh.write(f"\n[{phase}] ({len(msgs)} issues):\n")
            for m in msgs:
                fh.write(f"  • {m[:200]}\n")

    print(f"\n[{ts()}] Full log: {LOG_FILE}", flush=True)
    print(f"[{ts()}] Issues:  {ISSUE_FILE}", flush=True)


if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n[Interrupted by user]", flush=True)
        sys.exit(0)
